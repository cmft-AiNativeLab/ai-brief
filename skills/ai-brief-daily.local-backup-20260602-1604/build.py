...
def _screenshot(html_path, png_path, w, h, scale=2):
    """卡片截图 —— 优先用 Playwright Node 渲染，回退 Chrome。"""
    PW_RENDER = SCRIPTS / "pw-render.js"
    PW_ENV = {"LD_LIBRARY_PATH": "/home/node/.local/lib:/home/node/.local/lib-stubs:/home/node/.openclaw/workspace/node_modules/canvas/build/Release:" + os.environ.get("LD_LIBRARY_PATH", "")}
    if PW_RENDER.exists():
        cmd = ["node", str(PW_RENDER), "card", str(html_path.resolve()), str(png_path.resolve()), str(w), str(h), str(scale)]
        r = subprocess.run(cmd, capture_output=True, timeout=60, check=False, env={**os.environ, **PW_ENV})
    ...
    if pathlib.Path(CHROME_PATH).exists():
        ...
        subprocess.run(cmd, capture_output=True, timeout=60, check=False, env={**os.environ, **PW_ENV})
        return png_path.exists()
    print("[warn] No renderer available，跳过卡片截图", file=sys.stderr)
    return False
...
def build_downloads(payload, date):
    ...
    # 3) 卡片 PNG（3:4 分享卡，900×1200 窗口）
    card_png = dl / f"ai-brief-card-{date}.png"
    if _screenshot(DOCS / "card.html", card_png, 900, 1200, scale=2):
