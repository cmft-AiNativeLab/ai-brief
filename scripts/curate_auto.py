...
MAX_CAND = 80  # 传给 LLM 的候选上限（源增多后提高上限）
PER_SRC_CAP = 8  # 候选阶段每来源最多 N 条：避免高频源（如 Hacker News）淹没候选池导致选稿偏科
...
2. 源分布必须均衡：任一 source 最多 3 条（硬性约束，务必遵守），并尽量覆盖更多不同来源（理想 ≥8 个源，包括中英文）。
3. 每条写：
